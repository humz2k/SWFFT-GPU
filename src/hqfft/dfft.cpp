...
namespace HQFFT{
    ...
    int3 Dfft<Dist,REORDER_T,CollectiveComm,MPI_T,FFTBackend>::get_ks(int idx){
        if (k_in_blocks){
            ...
            int3 global_idx = make_int3(dist.local_coords_start[0] + local_idx.x, dist.local_coords_start[1] + local_idx.y, dist.local_coords_start[2] + local_idx.z);
            return global_idx;
        }
        else {
            ...
            int dest_x_start = 0;
            int dest_x_end = dist.dims[0] - 1;

            int y = ((dist.coords[0] * dist.dims[2] + dist.coords[2]) * (dist.ng[1] / (dist.dims[0] * dist.dims[2]))) / dist.local_grid_size[1];

            int y_send = dist.local_grid_size[1] / (dist.ng[1] / (dist.dims[0] * dist.dims[2]));
            int y_send_id = (((dist.coords[0] * dist.dims[2] + dist.coords[2]) * (dist.ng[1] / (dist.dims[0] * dist.dims[2]))) % dist.local_grid_size[1]) / y_send;

            int z = (dist.coords[1] * (dist.ng[2] / dist.dims[1])) / dist.local_grid_size[2];
            ...
            int3 global_idx = make_int3(x * dist.local_grid_size[0],y * dist.local_grid_size[1],z * dist.local_grid_size[2]);

            int dest = x*dist.dims[1]*dist.dims[2] + y*dist.dims[2] + z;

            //int reqidx = x - dest_x_start;
            //int nx = (dest_x_end+1) - dest_x_start;
            //int ny = n_recvs / nx;

            //int xsrc = x * dist.local_grid_size[0];
            int ysrc = ((dist.coords[0] * dist.dims[2] + dist.coords[2]) * (dist.ng[1] / (dist.dims[0] * dist.dims[2])));
            int zsrc = (dist.coords[1] * (dist.ng[2] / dist.dims[1]));

            int zoff = zsrc - dist.local_grid_size[2] * z;
            int yoff = (ysrc - dist.local_grid_size[1] * y) / (dist.ng[1] / (dist.dims[0] * dist.dims[2]));
            int tmp1 = zoff / (dist.ng[2] / dist.dims[1]);
            int new_count = tmp1 * y_send + yoff;
            
            int i = (new_idx % (dist.nlocal / n_recvs)) + new_count * (dist.nlocal / n_recvs);

            int ny = y_send;
            int nz = n_recvs / y_send;

            {
            int3 local_dims = make_int3(dist.local_grid_size[0],dist.local_grid_size[1] / ny,dist.local_grid_size[2] / nz); //per rank dims
