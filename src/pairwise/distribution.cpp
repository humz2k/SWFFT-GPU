...
#include "pairwise/distribution.hpp"
#include <cassert>
...
distribution_t<T, MPI_T>::distribution_t(MPI_Comm comm, int nx, int ny, int nz,
                                         bool debug_)
    : debug(debug_), n{nx, ny, nz}, parent(comm) {
    ...
    if (self == 0) {
        printf("distribution 1D: [%d:%d:%d]\n", process_topology_1.nproc[0],
               process_topology_1.nproc[1], process_topology_1.nproc[2]);
        fflush(stdout);
    }
#endif
    ...
    if (self == 0) {
        printf("distribution 3D: [%d:%d:%d]\n", process_topology_3.nproc[0],
               process_topology_3.nproc[1], process_topology_3.nproc[2]);
        fflush(stdout);
    }
#endif
    ...
    if (process_topology_2_z.n[0] != 0 && process_topology_2_z.n[1] != 0 &&
        process_topology_2_z.n[2] != 0) { // protects from dividing by zero.
        check_z_dims =
            ((process_topology_3.n[0]) % (process_topology_2_z.n[0]) == 0) &&
        ...
        if (self == 0 && debug && !check_z_dims) {
            fprintf(stderr,
                    "Need to fix Z PENCILS z_procs(%d,%d,%d) 3d.ns(%d,%d,%d) "
                    "2d_z.ns(%d,%d,%d).... \n",
                    process_topology_2_z.nproc[0],
                    process_topology_2_z.nproc[1],
                    process_topology_2_z.nproc[2], process_topology_3.n[0],
                    process_topology_3.n[1], process_topology_3.n[2],
                    process_topology_2_z.n[0], process_topology_2_z.n[1],
        ...
        if (!(check_z_dims) &&
            ...
            if (self == 0 && debug)
                fprintf(
                    stderr,
                    "Swaping Z pencils in initialization  (%d,%d,%d).... \n",
                    process_topology_2_z.nproc[0],
                    process_topology_2_z.nproc[1],
            ...
            process_topology_2_z.n[2] = n[2] / process_topology_2_z.nproc[2];
            check_z_dims =
                ((process_topology_3.n[0]) % (process_topology_2_z.n[0]) ==
                 0) &&
                ((process_topology_3.n[1]) % (process_topology_2_z.n[1]) ==
                 0) &&
                (n[0] % (process_topology_2_z.nproc[0]) == 0) &&
                (n[0] % (process_topology_2_z.nproc[1]) == 0);
        }
    ...
    /*
        if that did not work, make a pencil that does if inside the 3d cuboids
       by taking the cuboids dimensions (np1,np2,np3) and making pencils
    ...
    */
    if (!check_z_dims) {
        if (self == 0 && debug)
            fprintf(
                stderr,
                "MAKING Z PENCILS FIT zprocs(%d,%d,%d) z.ns(%d,%d,%d).... \n",
                process_topology_2_z.nproc[0], process_topology_2_z.nproc[1],
                process_topology_2_z.nproc[2], process_topology_2_z.n[0],
                process_topology_2_z.n[1], process_topology_2_z.n[2]);
        ...
        if (process_topology_3.n[0] > process_topology_3.n[1]) {
            ...
            if ((n[0] % (process_topology_2_z.nproc[0]) != 0) ||
                (n[0] % (process_topology_2_z.nproc[1]) != 0)) {
                process_topology_2_z.nproc[0] =
                    process_topology_3.nproc[0] * process_topology_3.nproc[2];
        ...
        if (self == 0 && debug)
            fprintf(stderr,
                    "MAKING Z PENCILS FIT AFTER zprocs(%d,%d,%d) "
                    "z.ns(%d,%d,%d)...\n",
                    process_topology_2_z.nproc[0],
                    process_topology_2_z.nproc[1],
                    process_topology_2_z.nproc[2], process_topology_2_z.n[0],
    ...
    if (debug && 0 == self) {
        ...
        for (int i = 0; i < ndim; ++i) {
            fprintf(stderr, "%d%s", process_topology_2_z.nproc[i],
                    separator(i, ndim));
        }
        fprintf(stderr, "\n");
    }
    if (!check_z_dims && debug && (self == 0)) {
        ...
        fprintf(outfile, "Z DIMS FAILS:(%d,%d,%d) (%d,%d,%d) \n",
                process_topology_2_z.nproc[0], process_topology_2_z.nproc[1],
                process_topology_2_z.nproc[2], process_topology_3.nproc[0],
                process_topology_3.nproc[1], process_topology_3.nproc[2]);
    }
    assert(check_z_dims);
    /*
    ...
    */
    MPI_Cart_create(comm, ndim, process_topology_2_z.nproc, period, 0,
    ...
    if (self == 0) {
        printf("distribution 2z: [%d:%d:%d]\n", process_topology_2_z.nproc[0],
               process_topology_2_z.nproc[1], process_topology_2_z.nproc[2]);
        fflush(stdout);
    }
#endif
    ...
    if (process_topology_2_x.n[0] != 0 && process_topology_2_x.n[1] != 0 &&
        process_topology_2_x.n[2] != 0) { // protects from dividing by zero.
        check_x_dims =
            ((process_topology_3.n[2]) % (process_topology_2_x.n[2]) == 0) &&
        ...
        if (self == 0 && debug && !check_x_dims)
            fprintf(stderr,
                    "Need to fix X PENCILS x_procs(%d,%d,%d) 3d.ns(%d,%d,%d) "
                    "2d_x.ns(%d,%d,%d)...\n",
                    process_topology_2_x.nproc[0],
                    process_topology_2_x.nproc[1],
                    process_topology_2_x.nproc[2], process_topology_3.n[0],
                    process_topology_3.n[1], process_topology_3.n[2],
                    process_topology_2_x.n[0], process_topology_2_x.n[1],
        ...
        if (!(check_x_dims) &&
            ...
            process_topology_2_x.n[2] = n[2] / process_topology_2_x.nproc[2];
            check_x_dims =
                ((process_topology_3.n[2]) % (process_topology_2_x.n[2]) ==
                 0) &&
                ((process_topology_3.n[1]) % (process_topology_2_x.n[1]) ==
                 0) &&
                (n[0] % (process_topology_2_x.nproc[2]) == 0) &&
                (n[0] % (process_topology_2_x.nproc[1]) == 0);
        }
    ...
    */
    if (!check_x_dims) {
        if (self == 0 && debug)
            fprintf(stderr,
                    "MAKING X PENCILS FIT xprocs(%d,%d,%d) x.ns(%d,%d,%d)...\n",
                    process_topology_2_x.nproc[0],
                    process_topology_2_x.nproc[1],
                    process_topology_2_x.nproc[2], process_topology_2_x.n[0],
                    process_topology_2_x.n[1], process_topology_2_x.n[2]);

        process_topology_2_x.nproc[0] = 1;
        if (process_topology_3.nproc[2] > process_topology_3.nproc[1]) {
            process_topology_2_x.nproc[1] =
                process_topology_3.nproc[1] * process_topology_3.nproc[0];
            ...
            if ((n[0] % (process_topology_2_x.nproc[2]) != 0) ||
                ...
                process_topology_2_x.nproc[2] =
                    process_topology_3.nproc[2] * process_topology_3.nproc[0];
        ...
        if (self == 0 && debug)
            fprintf(stderr,
                    "MAKING X PENCILS FIT AFTER xprocs(%d,%d,%d) "
                    "x.ns(%d,%d,%d)...\n",
                    process_topology_2_x.nproc[0],
                    process_topology_2_x.nproc[1],
                    process_topology_2_x.nproc[2], process_topology_2_x.n[0],
    ...
    if (debug && 0 == self) {
        fprintf(stderr, "  2d_x: ");
        for (int i = 0; i < ndim; ++i) {
            fprintf(stderr, "%d%s", process_topology_2_x.nproc[i],
                    separator(i, ndim));
        }
        fprintf(stderr, "\n");
    }
    if (!check_x_dims && debug && (self == 0)) {
        ...
        fprintf(outfile, "X DIMS FAILS:(%d,%d,%d) (%d,%d,%d) \n",
                process_topology_2_x.nproc[0], process_topology_2_x.nproc[1],
                process_topology_2_x.nproc[2], process_topology_3.nproc[0],
                process_topology_3.nproc[1], process_topology_3.nproc[2]);
    }
    assert(check_x_dims);
    /*
    ...
    */
    MPI_Cart_create(comm, ndim, process_topology_2_x.nproc, period, 0,
    ...
    if (process_topology_2_y.n[0] != 0 && process_topology_2_y.n[1] != 0 &&
        process_topology_2_y.n[2] != 0) { // protects from dividing by zero.
        check_y_dims =
            (((process_topology_3.n[2]) % (process_topology_2_y.n[2]) == 0) &&
        ...
        if (self == 0 && debug && !check_y_dims)
            fprintf(stderr,
                    "Need to fix Y PENCILS y_procs(%d,%d,%d) 3d.ns(%d,%d,%d) "
                    "2d_y.ns(%d,%d,%d)...\n",
                    process_topology_2_y.nproc[0],
                    process_topology_2_y.nproc[1],
                    process_topology_2_y.nproc[2], process_topology_3.n[0],
                    process_topology_3.n[1], process_topology_3.n[2],
                    process_topology_2_y.n[0], process_topology_2_y.n[1],
        ...
        if (!(check_y_dims) &&
            ...
            process_topology_2_y.n[2] = n[2] / process_topology_2_y.nproc[2];
            check_y_dims =
                (((process_topology_3.n[2]) % (process_topology_2_y.n[2]) ==
                  0) &&
                 ((process_topology_3.n[0]) % (process_topology_2_y.n[0]) ==
                  0) &&
                 (n[0] % (process_topology_2_y.nproc[2]) == 0) &&
                 (n[0] % (process_topology_2_y.nproc[0]) == 0));
        }
    ...
    */
    if (!check_y_dims) {
        if (self == 0 && debug)
            fprintf(stderr,
                    "MAKING Y PENCILS FIT yprocs(%d,%d,%d) y.ns(%d,%d,%d)...\n",
                    process_topology_2_y.nproc[0],
                    process_topology_2_y.nproc[1],
                    process_topology_2_y.nproc[2], process_topology_2_y.n[0],
                    process_topology_2_y.n[1], process_topology_2_y.n[2]);

        process_topology_2_y.nproc[1] = 1;
        if (process_topology_3.nproc[2] > process_topology_3.nproc[0]) {
            process_topology_2_y.nproc[0] =
                process_topology_3.nproc[0] * process_topology_3.nproc[1];
            process_topology_2_y.nproc[2] = process_topology_3.nproc[2];
            if ((n[0] % (process_topology_2_y.nproc[2]) != 0) ||
                (n[0] % (process_topology_2_y.nproc[0]) != 0)) {
                process_topology_2_y.nproc[2] =
                    process_topology_3.nproc[2] * process_topology_3.nproc[1];
        ...
        if (self == 0 && debug)
            fprintf(stderr,
                    "MAKING Y PENCILS FIT AFTER yprocs(%d,%d,%d) "
                    "y.ns(%d,%d,%d)...\n",
                    process_topology_2_y.nproc[0],
                    process_topology_2_y.nproc[1],
                    process_topology_2_y.nproc[2], process_topology_2_y.n[0],
    ...
    if (debug && 0 == self) {
        fprintf(stderr, "  2d_y: ");
        for (int i = 0; i < ndim; ++i) {
            fprintf(stderr, "%d%s", process_topology_2_y.nproc[i],
                    separator(i, ndim));
        }
        fprintf(stderr, "\n");
    }
    if (!check_y_dims && debug && (self == 0)) {
        ...
        fprintf(outfile, "Y DIMS FAILS:(%d,%d,%d) (%d,%d,%d) \n",
                process_topology_2_y.nproc[0], process_topology_2_y.nproc[1],
                process_topology_2_y.nproc[2], process_topology_3.nproc[0],
                process_topology_3.nproc[1], process_topology_3.nproc[2]);
    }
    assert(check_y_dims);
    /*
    ...
    points in that dimension, otherwise, this error will happen).
    */
    MPI_Cart_create(comm, ndim, process_topology_2_y.nproc, period, 0,
    ...
    if (self == 0) {
        printf("distribution 2y: [%d:%d:%d]\n", process_topology_2_y.nproc[0],
               process_topology_2_y.nproc[1], process_topology_2_y.nproc[2]);
        fflush(stdout);
    }
#endif
    ...
    if (debug) {
        ...
        if (myrank_z != self || myrank_y != self || myrank_x != self ||
            myrank_cube != self)
        ...
        for (int p = 0; p < nproc; ++p) {
            ...
            if (p == self) {
                fprintf(
                    stderr,
                    "  %d: 1d = (%d, %d, %d), 2d_x = (%d, %d, %d) rank is= "
                    "%d,2d_y = (%d, %d, %d) rank is= %d,2d_z = (%d, %d, %d) "
                    "rank is= %d, 3d = (%d, %d, %d). rank is= %d\n",
                    self, process_topology_1.self[0],
                    process_topology_1.self[1], process_topology_1.self[2],
                    process_topology_2_x.self[0], process_topology_2_x.self[1],
                    process_topology_2_x.self[2], myrank_x,
                    process_topology_2_y.self[0], process_topology_2_y.self[1],
                    process_topology_2_y.self[2], myrank_y,
                    process_topology_2_z.self[0], process_topology_2_z.self[1],
                    process_topology_2_z.self[2], myrank_z,
                    process_topology_3.self[0], process_topology_3.self[1],
                    process_topology_3.self[2], myrank_cube);
            }
        }
    }
    ...
    int buff_z_chunk = process_topology_2_z.n[0] * process_topology_2_z.n[1] *
                       process_topology_3.n[2];
    int buff_y_chunk = process_topology_2_y.n[0] * process_topology_2_y.n[2] *
                       process_topology_3.n[1];
    int buff_x_chunk = process_topology_2_x.n[1] * process_topology_2_x.n[2] *
...
template <class T, class MPI_T> distribution_t<T, MPI_T>::~distribution_t() {
...
template <class T>
void make_mpi_subarray(int ndims, const int* array_of_sizes,
                       const int* array_of_subsizes, const int* array_of_starts,
                       MPI_Datatype* new_dt);

template <>
void make_mpi_subarray<complexDoubleHost>(int ndims, const int* array_of_sizes,
                                          const int* array_of_subsizes,
                                          const int* array_of_starts,
                                          MPI_Datatype* new_dt) {
    MPI_Type_create_subarray(ndims, array_of_sizes, array_of_subsizes,
                             array_of_starts, MPI_ORDER_C, MPI_DOUBLE_COMPLEX,
                             new_dt);
    MPI_Type_commit(new_dt);
}

template <>
void make_mpi_subarray<complexFloatHost>(int ndims, const int* array_of_sizes,
                                         const int* array_of_subsizes,
                                         const int* array_of_starts,
                                         MPI_Datatype* new_dt) {
    MPI_Type_create_subarray(ndims, array_of_sizes, array_of_subsizes,
                             array_of_starts, MPI_ORDER_C, MPI_COMPLEX, new_dt);
    MPI_Type_commit(new_dt);
}

template <class T, class MPI_T>
void distribution_t<T, MPI_T>::redistribute(const T* a, T* b,
                                            redist_t direction) {
    int remaining_dim[3];
    ...
    for (int p = 0; p < subgrid_nproc; ++p) {
        ...
        if (0) {
            ...
            fprintf(stderr,
                    "%d: d1_peer, d1_coord, d3_peer = %d, (%d, %d), %d\n", self,
                    d1_peer, coord[0], coord[1], d3_peer);
        }

        // create dataypes representing a subarray in the 1- and 3-d
        // distributions
        ...
        // MPI_DOUBLE_COMPLEX, &d1_type); MPI_Type_commit(&d1_type);
        ...
        // MPI_Type_create_subarray(3, sizes, subsizes, starts, MPI_ORDER_C,
        // MPI_DOUBLE_COMPLEX, &d3_type); MPI_Type_commit(&d3_type);

        // exchange data

        if (direction == REDISTRIBUTE_3_TO_1) {
            MPI_Sendrecv((char*)a, 1, d3_type, d3_peer, 0, (char*)b, 1, d1_type,
                         d1_peer, 0, subgrid_cart, MPI_STATUS_IGNORE);
        } else if (direction == REDISTRIBUTE_1_TO_3) {
            MPI_Sendrecv((char*)a, 1, d1_type, d1_peer, 0, (char*)b, 1, d3_type,
                         d3_peer, 0, subgrid_cart, MPI_STATUS_IGNORE);
        } else {
            abort();
        }
...
template <class T, class MPI_T>
void distribution_t<T, MPI_T>::redistribute_slab(const T* a, T* b,
                                                 redist_t direction) {
    int remaining_dim[3];
    MPI_Comm subgrid_cart;
    int subgrid_self;
    int subgrid_nproc;
    ptrdiff_t d1_slice =
        process_topology_1.n[1] * process_topology_1.n[2] * sizeof(T);
    ptrdiff_t d3_slice =
        process_topology_3.n[1] * process_topology_3.n[2] * sizeof(T);
    ...
    for (int p = 0; p < subgrid_nproc; ++p) {
        ...
        for (int slice = 0; slice < process_topology_1.n[0]; ++slice) {
            ...
            ptrdiff_t d3_offset =
                (slice + d3_peer * process_topology_1.n[0]) * d3_slice;

            // create subarray dataypes representing the slice subarray in the
            // 1- and 3-d distributions
            ...
            /*MPI_Type_contiguous(process_topology_3.n[1] *
               process_topology_3.n[2], MPI_DOUBLE_COMPLEX, &d3_type);*/
            MPI_Type_contiguous(process_topology_3.n[1] *
                                    process_topology_3.n[2] * sizeof(T),
                                MPI_BYTE, &d3_type);
...
template <class T, class MPI_T>
void distribution_t<T, MPI_T>::redistribute_2_and_3(const T* a, T* b,
                                                    redist_t direction,
                                                    int z_dim) {
    ...
    bool print_mess =
        false; // prints communication sends and recieves without actually doing
               // the comms(intended to debug comm hangs).
    bool print_result =
        false /*true*/; // prints a line in a file called "passed.data" which
                        // happends if the code runs completly.
    assert(z_dim == 0 || z_dim == 1 || z_dim == 2);
    int x_dim = 0, y_dim = 0;
    // x_dim, y_dim and z_dim are the dimensions of the x,y,z axis of the pencil
    // with respect to the original axis(where index 2 is into the grid, 1 is
    // vertical translation and 0 is horizontal).
    switch (z_dim) {
    case 0:
        x_dim = 1;
        y_dim = 2;
        if ((self == me) && print_me)
            fprintf(stderr, "DOING X PENCILS!...\n");
        break;
    case 1:
        x_dim = 2;
        y_dim = 0;
        if ((self == me && print_me))
            fprintf(stderr, "DOING Y PENCILS!...\n");
        break;
    case 2:
        x_dim = 0;
        y_dim = 1;
        if ((self == me && print_me))
            fprintf(stderr, "DOING Z PENCILS!...\n");
        break;
    default:
        assert("incorrect inputed dimension");
    }
    ...
    npeers =
        process_topology_3
            .nproc[z_dim]; // picked last direction (lets say into the grid)
    ...
    switch (z_dim) {
    case 0:
        p1max = process_topology_2_x.nproc[x_dim] /
                    process_topology_3.nproc[x_dim] -
                1;
        // find out the size of the chunk you need to use (stored in subsizes),
        // and set sizes to the local size of the pencil. The x and y dimensions
        // of the subchunck will be the dimensions of the pencil (since the code
        // asserts at the beginning that all pencils fit inside the 3d cuboid.)
        // The z dimension will be the dimension of the cuboid, since this will
        // always be <= to the z_dim of the pencil.
        ...
        break;
    case 1:
        p1max = process_topology_2_y.nproc[x_dim] /
                    process_topology_3.nproc[x_dim] -
                1;
        ...
        break;
    case 2:
        p1max = process_topology_2_z.nproc[y_dim] /
                    process_topology_3.nproc[y_dim] -
                1;
    ...
    int chunk_size =
        ...
        subsizes[2]; // size of data chunks that will be communicated between
                     // pencil and cube distributions.

    // set variables that will be used to find pencils chunks
    int pencil_dims[3] = {0, 0,
                          0}; // size of entire pencil in its local coord system
    ...
    if (z_dim == 2) {
        ...
        pencil_dims[0] =
            process_topology_2_z.n[0]; // pencil dims in grid coord system
                                       // (where index 2 is the z direction).
    ...
    for (int p = 0; p < npeers; ++p) {
        if ((self == me) && print_me)
            fprintf(stderr, "%d, %d, %d Made it beg-for!...\n", self, p,
                    npeers);
        ...
        // turn the processor coordinate into one specified by the number of
        // data points in each dimension.
        for (int i = 0; i < 3; ++i) {
            switch (z_dim) {
            case 0:
                d2_coord[i] =
                    process_topology_2_x.self[i] * process_topology_2_x.n[i];
                break;
            case 1:
                d2_coord[i] =
                    process_topology_2_y.self[i] * process_topology_2_y.n[i];
                break;
            case 2:
                d2_coord[i] =
                    process_topology_2_z.self[i] * process_topology_2_z.n[i];
                break;
            }
        }
        // over every iteration of the loop, transverse down the pencil (since
        // it will be divided in chunks whose coordinates will only differ in
        // the z_dimension.
        d2_coord[z_dim] += p * process_topology_3.n[z_dim];

        if ((self == me) && print_me)
            fprintf(stderr, "%d, %d, %d Coord!...\n", d2_coord[0], d2_coord[1],
                    d2_coord[2]);

        // d2_array_start is the starting index of the chunk in the pencils
        // local coordinates.
        ...
        if (DEBUG_CONDITION || ((self == me) && print_me)) {
            fprintf(stderr,
                    "%d: pencil_sizes=(%d,%d,%d), cube_sizes=(%d,%d,%d), "
                    "subsizes=(%d,%d,%d),d2_coord=(%d,%d,%d), "
                    "d2_array_start=(%d,%d,%d) \n",
                    self, pencil_sizes[0], pencil_sizes[1], pencil_sizes[2],
                    cube_sizes[0], cube_sizes[1], cube_sizes[2], subsizes[0],
                    subsizes[1], subsizes[2], d2_coord[0], d2_coord[1],
                    d2_coord[2], d2_array_start[0], d2_array_start[1],
                    d2_array_start[2]);
        }

        // if making cuboids from pencils, right here we need to fill the
        // d2_chunk array with the data that later needs to be sent to a cuboid.
        // The array is a chunk of the pencil and is why we needed to calculate
        // the starting index for the array in the local coordinates.
...
#pragma omp parallel for schedule(static) firstprivate(ch_indx, last_i)
            ...
            for (int i0 = d2_array_start[0];
                 i0 < d2_array_start[0] + local_sizes[0]; i0++) {

#ifdef DIST_OMP
                if (last_i != i0 - 1) {
                    ch_indx = 0;
                    for (int i = d2_array_start[0]; i < i0; i++)
                        for (int i1 = d2_array_start[1];
                             i1 < d2_array_start[1] + local_sizes[1]; i1++)
                            for (int i2 = d2_array_start[2];
                                 i2 < d2_array_start[2] + local_sizes[2]; i2++)
                                ch_indx++;
                }
                last_i = i0;
#endif
                ...
                for (int i1 = d2_array_start[1];
                    ...
                    for (int i2 = d2_array_start[2];
                         i2 < d2_array_start[2] + local_sizes[2]; i2++) {
                        int64_t local_indx =
                            pencil_dims[2] * (pencil_dims[1] * i0 + i1) + i2;
#ifdef DIST_DO_ASSERT
                        assert(local_indx < dims_size);
                        assert(ch_indx < chunk_size && ch_indx >= 0 &&
                               local_indx >= 0 && local_indx < dims_size);
#endif
        ...
        if ((self == me) && print_me)
            fprintf(stderr, "%d, %d, %d Cube that hits pencil coord!...\n",
                    d3_peer_coord[0], d3_peer_coord[1], d3_peer_coord[2]);
        // find the rank of this peer.
        switch (z_dim) {
        case 0:
            MPI_Cart_rank(process_topology_3.cart, d3_peer_coord, &d3_peer);
            break;
        case 1:
            MPI_Cart_rank(process_topology_3.cart, d3_peer_coord, &d3_peer);
            break;
        ...
        if ((self == me) && print_me)
            fprintf(stderr, "%d, %d, PEER!...\n", self, d3_peer);

        // By here in the for loop, we have broken the pencil into a chunk and
        // found which cuboid it resides; over every iteration, the for-loop
        // will break up the pencil in the z_dimension. From here on we do the
        // opposite. We divide the cuboid into chunks (that are the same size as
        // the ones in the pencil), and determine which pencils own these
        // chunks.
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            case 0:
                d3_coord[i] =
                    process_topology_3.self[i] * process_topology_3.n[i];
                break;
            case 1:
                d3_coord[i] =
                    process_topology_3.self[i] * process_topology_3.n[i];
        ...
        // now unlike above, we dont need to iterate in the z_dim, because for
        // each processor its subarrays inward dimension is already set by the
        // cubes z_dim. Instead, each iteration of the for-loop will look at
        // different subarrays whose locations in the cuboid differ by local x
        // and y coords.

        switch (z_dim) {
        // p1 is a place holder for the first translation . The outside for-loop
        // will increment the coord in that direction, say x_dim, and keep doing
        // so until all of the chunks in that dimension are calculated. Then it
        // will increment p0 in the other dimension (in this example the y) and
        // repeat until all of the subchunks in the x and y dimensions are
        // calculated. are found. Note: p0 and p1 will increment different
        // dimensions depending of whether it is using the x y or z pencils,
        // this is because the set up of the coordinate system for each pencil
        // is different and to ensure that no communications hang up later, the
        // directions coded below are unique for each type of pencil.
        case 0:
        ...
        // d3_array_start holds the starting index of the chunk in the cubes
        // local coordinates(note the cubes local coord system is actually the
        // same as the grids global coord system, by set up)
        ...
        // make starting point so that it coincides with the starting point of
        // the pencil from the pencils coordinate system. (for z_pencils nothing
        // needs to be changed, since it already has the coordinate system of
        // the grid, however, the x and y pencils have different starting points
        // of the subchunk in their coord systems.)
        if (z_dim == 0 || z_dim == 1) {
        ...
        if (print_me && (self == me))
            fprintf(
                stderr,
                "D3_array_start is (%d,%d,%d) and subsizes is (%d,%d,%d) \n",
                d3_array_start[0], d3_array_start[1], d3_array_start[2],
                subsizes[0], subsizes[1], subsizes[2]);

        // If sending cube chunks to pencils, need to fill those chunks with
        // data here. The chunks are filled in the order such that when the
        // pencil recieves the chunk, in its local array indexing, it assumes
        // that the array is already filled such that it is contiguous.
        // Therefore, complicated for-loops below fill the array in the cubes
        // local indexing to match what the pencil will expect.
...
#pragma omp parallel for schedule(static) firstprivate(ch_indx, last_i)
#endif
                for (int i2 = d3_array_start[y_dim];
                     i2 > d3_array_start[y_dim] - subsizes[y_dim];
                     i2--) { // perhaps y_dim
                    ...
                    if (last_i != i2 + 1) {
                        ...
                        for (int i = d3_array_start[y_dim]; i > i2; i--)
                            for (int i1 = d3_array_start[x_dim];
                                 i1 < d3_array_start[x_dim] + subsizes[x_dim];
                                 i1++)
                                for (int i0 = d3_array_start[z_dim];
                                     i0 <
                                     d3_array_start[z_dim] + subsizes[z_dim];
                                     i0++)
                                    ch_indx++;
                    }
                    last_i = i2;
                    ...
                    for (int i1 = d3_array_start[x_dim];
                         ...
                         i1++) { // perhaps x_dim
                        for (int i0 = d3_array_start[z_dim];
                             i0 < d3_array_start[z_dim] + subsizes[z_dim];
                             i0++) { // perhaps z_dim
                            int64_t local_indx =
                                process_topology_3.n[2] *
                                    (process_topology_3.n[1] * i0 + i1) +
                                i2;
#ifdef DIST_DO_ASSERT
                            assert(local_indx < dims_size);
                            assert(ch_indx < chunk_size && ch_indx >= 0 &&
...
#pragma omp parallel for schedule(static) firstprivate(ch_indx, last_i)
                ...
                for (int i0 = d3_array_start[y_dim];
                     i0 < d3_array_start[y_dim] + subsizes[y_dim]; i0++) {

#ifdef DIST_OMP
                    if (last_i != i0 - 1) {
                        ch_indx = 0;
                        for (int i = d3_array_start[y_dim]; i < i0; i++)
                            for (int i2 = d3_array_start[x_dim];
                                 i2 > d3_array_start[x_dim] - subsizes[x_dim];
                                 i2--)
                                for (int i1 = d3_array_start[z_dim];
                                     i1 <
                                     d3_array_start[z_dim] + subsizes[z_dim];
                                     i1++)
                                    ch_indx++;
                    ...
                    for (int i2 = d3_array_start[x_dim];
                         i2 > d3_array_start[x_dim] - subsizes[x_dim]; i2--) {
                        for (int i1 = d3_array_start[z_dim];
                             i1 < d3_array_start[z_dim] + subsizes[z_dim];
                             i1++) {
                            int64_t local_indx =
                                process_topology_3.n[2] *
                                    (process_topology_3.n[1] * i0 + i1) +
...
#pragma omp parallel for schedule(static) firstprivate(ch_indx, last_i)
                ...
                for (int i0 = d3_array_start[x_dim];
                     i0 < d3_array_start[x_dim] + subsizes[x_dim]; i0++) {

#ifdef DIST_OMP
                    if (last_i != i0 - 1) {
                        ch_indx = 0;
                        for (int i = d3_array_start[x_dim]; i < i0; i++)
                            for (int i1 = d3_array_start[y_dim];
                                 i1 < d3_array_start[y_dim] + subsizes[y_dim];
                                 i1++)
                                for (int i2 = d3_array_start[z_dim];
                                     i2 <
                                     d3_array_start[z_dim] + subsizes[z_dim];
                                     i2++)
                                    ch_indx++;
                    ...
                    for (int i1 = d3_array_start[y_dim];
                         i1 < d3_array_start[y_dim] + subsizes[y_dim]; i1++) {
                        for (int i2 = d3_array_start[z_dim];
                             i2 < d3_array_start[z_dim] + subsizes[z_dim];
                             i2++) {
                            int64_t local_indx =
                                process_topology_3.n[2] *
                                    (process_topology_3.n[1] * i0 + i1) +
        ...
        if (DEBUG_CONDITION || ((self == me) && print_me)) {
            fprintf(stderr,
                    "%d: pencil_sizes=(%d,%d,%d), cube_sizes=(%d,%d,%d), "
                    "subsizes=(%d,%d,%d), d3_coord=(%d,%d,%d), "
                    "d3_array_start=(%d,%d,%d) \n",
                    self, pencil_sizes[0], pencil_sizes[1], pencil_sizes[2],
                    cube_sizes[0], cube_sizes[1], cube_sizes[2], subsizes[0],
                    subsizes[1], subsizes[2], d3_coord[0], d3_coord[1],
                    d3_coord[2], d3_array_start[0], d3_array_start[1],
                    d3_array_start[2]);
        }
        ...
        d2_peer_coord[z_dim] = 0; // since these are pencils, there is no two
                                  // pencils in this direction.
        if ((self == me) && print_me)
            fprintf(stderr, "%d, %d, %d PENCIL that hits chunk!...\n",
        ...
        // if print_mess boolean is set to true, then the code runs without
        // sending any messages, and is used to test which messages would be
        // sent in the entire run. (designed to debug comm hangups, if they
        // occur).

        if (direction == REDISTRIBUTE_3_TO_2) {

            if ((self == me) && print_mess)
                fprintf(stderr,
                        " I am %d, making request to recieve from %d...\n",
                        self, recv_peer);
            // if(!print_mess)MPI_Irecv((void *) d2_chunk, chunk_size,
            // MPI_DOUBLE_COMPLEX, recv_peer, 0, process_topology_1.cart,
            // &req1);
            if (!print_mess)
                MPI_Irecv((void*)d2_chunk, chunk_size * sizeof(T), MPI_BYTE,
                          recv_peer, 0, process_topology_1.cart, &req1);

            if ((self == me) && print_mess)
                fprintf(stderr, " I am %d, making request to send to %d...\n",
                        self, send_peer);
            // if(!print_mess)MPI_Isend((void *) d3_chunk, chunk_size,
            // MPI_DOUBLE_COMPLEX, send_peer, 0, process_topology_1.cart,
            // &req2);
            if (!print_mess)
                MPI_Isend((void*)d3_chunk, chunk_size * sizeof(T), MPI_BYTE,
                          send_peer, 0, process_topology_1.cart, &req2);
            ...
            // if((self == me || self == 1 || self == 2 || self == 3) &&
            // print_me)fprintf(stderr, " I am %d, waiting to send to %d...\n",
            // self,send_peer); fprintf(stderr, " I am %d, waiting to send to
            // %d...\n", self,send_peer);
            if (self == me && print_mess)
...
#endif
            if (self == me && print_me)
                fprintf(stderr, "REAL SUBSIZES (%d,%d,%d)\n", subsizes[x_dim],
                        subsizes[y_dim], subsizes[z_dim]);
            if (self == me && print_me)
                fprintf(stderr,
                        "PENCIL DIMENSION VS. local sizes (%d,%d,%d) vs "
                        "(%d,%d,%d)\n",
                        pencil_dims[0], pencil_dims[1], pencil_dims[2],
                        local_sizes[0], local_sizes[1], local_sizes[2]);
            if (self == me && print_me)
                fprintf(stderr, "DIM_2_ARRAY_START (%d,%d,%d) \n",
                        d2_array_start[0], d2_array_start[1],
                        d2_array_start[2]);
...
// if(local_indx >= dims_size)fprintf(stderr,"WOW, in third for, dims is (%d),
// we are %d and my rank is %d",dims_size,local_indx,self);
#ifdef DIST_DO_ASSERT
                        assert(local_indx < dims_size);
                        assert(ch_indx < chunk_size && ch_indx >= 0 &&
                               local_indx >= 0 && local_indx < dims_size);
                        ...
                        // self==me)fprintf(stderr,"(%f,%f)
                        // ",real(d2_chunk[ch_indx]),imag(d2_chunk[ch_indx]));
                        ch_indx++;
                    }
                }
            }
            //     if((p==0 ||p==1 || p==2 || p==3 || p==4 || p==5) &&
            //     self==me)fprintf(stderr,"P is %d \n",p);
            ...
            if ((self == me) && print_mess)
                fprintf(stderr,
                        " I am %d, making request to recieve from %d...\n",
                        self, recv_peer);
            // if(!print_mess)MPI_Irecv((void *) d3_chunk, chunk_size,
            // MPI_DOUBLE_COMPLEX, recv_peer, 0, process_topology_1.cart,
            // &req1);
            if (!print_mess)
                MPI_Irecv((void*)d3_chunk, chunk_size * sizeof(T), MPI_BYTE,
                          recv_peer, 0, process_topology_1.cart, &req1);

            if ((self == me) && print_mess)
                fprintf(stderr, " I am %d, making request to send to %d...\n",
                        self, send_peer);
            // if(!print_mess)MPI_Isend((void *) d2_chunk, chunk_size,
            // MPI_DOUBLE_COMPLEX, send_peer, 0, process_topology_1.cart,
            // &req2);
            if (!print_mess)
                MPI_Isend((void*)d2_chunk, chunk_size * sizeof(T), MPI_BYTE,
                          send_peer, 0, process_topology_1.cart, &req2);
            ...
            int64_t ch_indx = 0;
#ifdef DIST_DO_ASSERT
            int dims_size = (process_topology_3.n[2]) *
                            (process_topology_3.n[1]) *
                            (process_topology_3.n[0]);
#endif
...
#pragma omp parallel for schedule(static) firstprivate(ch_indx, last_i)
                ...
                for (int i2 = d3_array_start[y_dim];
                     i2 > d3_array_start[y_dim] - subsizes[y_dim]; i2--) {
                    ...
                    for (int i1 = d3_array_start[x_dim];
                         i1 < d3_array_start[x_dim] + subsizes[x_dim]; i1++) {
                        for (int i0 = d3_array_start[z_dim];
                             i0 < d3_array_start[z_dim] + subsizes[z_dim];
                             i0++) {
                            int64_t local_indx =
                                process_topology_3.n[2] *
                                    (process_topology_3.n[1] * i0 + i1) +
                                i2;
// if(local_indx >= dims_size)fprintf(stderr,"WOW, in fourth for, dims is (%d),
// we are %d and my rank is %d",dims_size,local_indx,self);
#ifdef DIST_DO_ASSERT
                            assert(local_indx < dims_size);
                            assert(ch_indx < chunk_size && ch_indx >= 0 &&
                            ...
                            //                         if(p==3 &&
                            //                         self==me)fprintf(stderr,"(%f,%f)
                            //                         ",real(d3_chunk[ch_indx]),imag(d3_chunk[ch_indx]));
                            ch_indx++;
                        }
                    }
                }
...
#pragma omp parallel for schedule(static) firstprivate(ch_indx, last_i)
        ...
        if (DEBUG_CONDITION) {
            fprintf(stderr,
                    ...
                    self, npeers, p, p0, p1, p1max, d3_coord[0], d3_coord[1],
                    d3_coord[2], d2_peer_coord[0], d2_peer_coord[1],
                    d2_peer_coord[2], d2_coord[0], d2_coord[1], d2_coord[2],
                    d3_peer_coord[0], d3_peer_coord[1], d3_peer_coord[2],
                    recv_peer, send_peer);
        }

        if ((self == me) && print_me)
            fprintf(stderr, "%d, %d, %d Made it end-for!...\n", self, p,
                    npeers);
    }

    // if((self == me) && print_me)fprintf(outfile, "   Made it all the way! for
    // z_dim =(%d) and num_proc = (%d)...\n", z_dim,
    // process_topology_1.nproc[0]);
    if ((self == me) && print_result) {
        ...
        if (outfile)
            fprintf(outfile,
                    "   Made it all the way! for z_dim =(%d) and num_proc = "
                    "(%d)...\n",
...
void distribution_t<T, MPI_T>::coord_cube(int myrank, int coord[]) {
    coord[0] =
        myrank / (process_topology_3.nproc[1] * process_topology_3.nproc[2]);
    coord[1] =
        (myrank % (process_topology_3.nproc[1] * process_topology_3.nproc[2])) /
...
void distribution_t<T, MPI_T>::rank_cube(int* my_rank, int coord[]) {
    *my_rank =
        coord[2] + (process_topology_3.nproc[2]) *
                       (coord[1] + process_topology_3.nproc[1] * coord[0]);
    return;
}
...
void distribution_t<T, MPI_T>::coord_x_pencils(int myrank, int coord[]) {
    ...
    coord[0] = 0;
    int num_pen_in_cube_col =
        process_topology_2_x.nproc[1] / process_topology_3.nproc[1];
    int num_pen_in_cube_row =
        process_topology_2_x.nproc[2] / process_topology_3.nproc[2];
    ...
    */
    ...
    coord[2] = (low_rank % process_topology_3.nproc[2]) * num_pen_in_cube_row +
               num_repeats / num_pen_in_cube_col;
...
void distribution_t<T, MPI_T>::rank_x_pencils(int* myrank, int coord[]) {
    ...
    *myrank = (alpha * num_cubes) +
              ...
              (beta * (num_cubes)*num_pen_in_cube_col) +
...
void distribution_t<T, MPI_T>::rank_y_pencils(int* myrank, int coord[]) {
    ...
    // if(num_pen_in_cube_col ==0)fprintf(stderr,"num_cube_col%d ",
    // process_topology_2_y.nproc[1]/process_topology_3.nproc[1]);
    // if(num_pen_in_cube_row ==0)fprintf(stderr,"num_cube_row%d ",
    // process_topology_3.nproc[2]);
    assert(num_pen_in_cube_col != 0 && num_pen_in_cube_row != 0);
    int beta = coord[2] % num_pen_in_cube_row;
...
void distribution_t<T, MPI_T>::coord_z_pencils(int myrank, int coord[]) {
    ...
    coord[0] = alpha * num_pen_in_cube_row +
               (myrank % num_pen_in_cube) / num_pen_in_cube_col;
    coord[1] =
        ((myrank % (process_topology_2_z.nproc[1] * num_pen_in_cube_row)) /
         num_pen_in_cube) *
            num_pen_in_cube_col +
        myrank % num_pen_in_cube_col;
...
void distribution_t<T, MPI_T>::rank_z_pencils(int* myrank, int coord[]) {
    ...
    *myrank = alpha + ((coord[1] / num_pen_in_cube_col) * num_pen_in_cube) +
              (beta * num_pen_in_cube_col) +
              (coord[0] / num_pen_in_cube_row) * process_topology_2_z.nproc[1] *
                  num_pen_in_cube_row;
    return;
}
